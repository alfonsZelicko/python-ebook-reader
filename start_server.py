...
Examples:
  ...
  python start_server.py --generate-env
    ...
    parser.add_argument(
        "--host", type=str, help="Server host address (overrides .env.server)"
    )

    parser.add_argument("--port", type=int, help="Server port (overrides .env.server)")

    parser.add_argument(
        "--generate-env", action="store_true", help="Generate .env.server template file"
    )

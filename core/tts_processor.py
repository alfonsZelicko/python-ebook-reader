...
def export_audiobook(
    ...
    for i, chunk in enumerate(tqdm.tqdm(chunks_to_process, desc="Export Progress")):
        ...
        try:
            ...
            if (
                len(current_segment_audio) + len(new_audio_chunk) > max_duration_ms
                and len(current_segment_audio) > 0
            ):
                # SAVE
                ...
                current_segment_audio.export(
                    output_filename, format="mp3", bitrate="192k"
                )
    ...
    if len(current_segment_audio) > 0:
        ...
        manager.update_state(
            last_chunk_index=total_chunks - 1, last_mp3_index=current_mp3_index
        )

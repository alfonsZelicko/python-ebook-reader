...
from server.services.job_manager import JobManager, Job
from server.types.outputs import JobStatusEnum, JobProgress
...
async def test_job_creation(job_manager):
    ...
    assert job.status in [JobStatusEnum.QUEUED, JobStatusEnum.RUNNING, JobStatusEnum.COMPLETED]
...
async def test_get_job_status_valid_id(job_manager):
    ...
    assert status.status in [JobStatusEnum.QUEUED, JobStatusEnum.RUNNING, JobStatusEnum.COMPLETED]
...
async def test_failed_job_handling(job_manager):
    ...
    job_id = await job_manager.create_job(
        job_type="TTS",
        service_func=failing_service_func,
        input_data={}
    )
...
async def test_cleanup_does_not_remove_running_jobs(job_manager):
    ...
    job_id = await job_manager.create_job(
        job_type="TTS",
        service_func=slow_service_func,
        input_data={}
    )

...
Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7, 13.1
"""
...
from server.types.outputs import TTSResult, TTSMetadata
...
class TTSService:
    ...
    async def generate_speech(
        ...
        try:
            ...
            self.logger.error(
                ...
                exc_info=True,
                extra={
                    'engine': input_data.engine if hasattr(input_data, 'engine') else 'unknown'
                }
            )
            ...
            if temp_input_file and os.path.exists(temp_input_file):
                try:
                    ...
                    self.logger.debug(f"Cleaned up temporary input file: {temp_input_file}")
                except Exception as e:
                    self.logger.warning(f"Failed to cleanup temp file: {temp_input_file} - {str(e)}")
    ...
    def _validate_engine(self, engine: str) -> None:
        ...
        if engine not in self.config.allowed_tts_engines:
            ...
            self.logger.warning(
                ...
                extra={'requested_engine': engine, 'allowed_engines': self.config.allowed_tts_engines}
            )
            raise ValueError(error_msg)
    ...
    async def _prepare_input_file(self, input_data) -> str:
        ...
        has_upload = hasattr(input_data, 'file_upload') and input_data.file_upload is not None
    ...
    def _convert_to_args(self, input_data, input_file: str) -> argparse.Namespace:
        ...
        args.CP = input_data.chunk_by_paragraph if hasattr(input_data, 'chunk_by_paragraph') else False
        args.SR = input_data.speaking_rate if hasattr(input_data, 'speaking_rate') else 1.1
        
        # Output configuration
        args.OT = input_data.output_type if hasattr(input_data, 'output_type') else "FILE"
        args.MFD = input_data.max_file_duration if hasattr(input_data, 'max_file_duration') else 600
        args.COD = input_data.clean_output_directory if hasattr(input_data, 'clean_output_directory') else False
        
        # Offline engine parameters
        args.OFF_VOICE = input_data.offline_voice if hasattr(input_data, 'offline_voice') else ""
        
        # Language configuration
        args.L_CODE = input_data.language_code if hasattr(input_data, 'language_code') else "cs-CZ"
        
        # Google Cloud parameters
        args.G_CRED = input_data.google_credentials if hasattr(input_data, 'google_credentials') else "./google-key.json"
        args.G_VOICE = input_data.google_voice if hasattr(input_data, 'google_voice') else "cs-CZ-Standard-B"
        
        # Coqui parameters
        args.C_MODEL = input_data.coqui_model if hasattr(input_data, 'coqui_model') else "tts_models/multilingual/multi-dataset/xtts_v2"
        args.C_SPEAKER = input_data.coqui_speaker if hasattr(input_data, 'coqui_speaker') else ""
        args.C_WAV = input_data.coqui_wav if hasattr(input_data, 'coqui_wav') else ""
        args.C_RATE = input_data.coqui_sample_rate if hasattr(input_data, 'coqui_sample_rate') else 22050
    ...
    def _collect_output_files(self, input_file: str, args: argparse.Namespace) -> List[str]:
        """
    ...
    def _calculate_total_duration(self, output_files: List[str]) -> float:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to calculate total duration: {str(e)}",
                extra={'error': str(e)}
            )
            return 0.0

...
from server.types.outputs import TranslationResult, TranslationMetadata
...
class TranslationService:
    ...
    def _validate_engine(self, engine: str) -> None:
        ...
        if engine not in self.config.allowed_translator_engines:
            ...
            self.logger.warning(
                ...
                extra={'requested_engine': engine, 'allowed_engines': self.config.allowed_translator_engines}
            )
            raise ValueError(error_msg)
    ...
    def _convert_to_args(self, input_data, input_file: str) -> argparse.Namespace:
        ...
        args.SL = input_data.source_language if hasattr(input_data, 'source_language') else "en"
        args.TL = input_data.target_language if hasattr(input_data, 'target_language') else "cs"
        args.TP = input_data.translation_prompt if hasattr(input_data, 'translation_prompt') else "You are a professional book translator. Translate the following fantasy text accurately while preserving the style and tone."
        args.CS = input_data.chunk_size if hasattr(input_data, 'chunk_size') else 4000
        args.CP = input_data.chunk_by_paragraph if hasattr(input_data, 'chunk_by_paragraph') else True
        
        # OpenAI parameters
        args.O_KEY = input_data.openai_api_key if hasattr(input_data, 'openai_api_key') else ""
        args.O_MODEL = input_data.openai_model if hasattr(input_data, 'openai_model') else "gpt-4o-mini"
        ...
        args.G_MODEL = input_data.gemini_model if hasattr(input_data, 'gemini_model') else "gemini-pro"
        
        # DeepL parameters
        args.D_KEY = input_data.deepl_api_key if hasattr(input_data, 'deepl_api_key') else ""
    ...
    def _estimate_chunks(self, output_file: str) -> int:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to estimate chunks: {str(e)}",
                extra={'error': str(e)}
            )
            return 0
